class CsatSurveys::ResponseBuilder
  ...
  def perform
    ...
    rating_technology = message.content_attributes.dig('submitted_values', 'csat_survey_response', 'rating_technology')
    feedback_message = message.content_attributes.dig('submitted_values', 'csat_survey_response', 'feedback_message')

    return if rating.blank?
    return if rating_technology.blank?

    process_csat_response(conversation, rating, rating_technology, feedback_message)
  end

  private

  def process_csat_response(conversation, rating, rating_technology, feedback_message)
    ...
    csat_survey_response.rating_technology = rating_technology
